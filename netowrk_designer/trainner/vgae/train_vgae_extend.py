...
def train_autoencoder(m1, trainloader, epoch_step, epoch, num_epochs, lr, optimizer, criterion, dist_loss, cond_loss=nn.MSELoss()):
    ...
    #for batch_idx, ((adj, ops), cond) in enumerate(trainloader):
       # print(cond)
        ...
        sys.stdout.write('\r')
        sys.stdout.write('| Epoch [%3d/%3d] Iter[%3d/%3d]\t\tLoss: %.4f \t\t Labelled_Rec_Loss: %.4f \t\tTriplet_loss %.4f \t\t UnLabelled_Rec_Loss %.4f \t\t Pred_loss %.4f '
                         % (epoch, num_epochs, batch_idx+1,
                            (epoch_step)+1, loss.data[0], rec_loss, triplet_loss, un_rec_loss, pred_loss))
        sys.stdout.flush()
