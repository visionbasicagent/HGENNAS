...
def train_autoencoder(m1, trainloader, epoch_step, epoch, num_epochs, lr, optimizer, criterion, dist_loss, cond_loss=nn.MSELoss()):
    ...
    for batch_idx, (((adj, ops), cond), ((un_adj, un_ops), _))in enumerate(trainloader):
       # print(cond)
        ...
        loss = rec_loss + dist_loss(mu, cond) + un_rec_loss + cond_loss(pred_cond, cond)
        #loss = rec_loss  + un_rec_loss
...
def test_autoencoder(m1, test_loder, test_set, epoch, criterion, dist_loss, cond_loss=nn.MSELoss()):
    ...
    for batch_idx,  ((adj, ops), cond) in enumerate(tqdm(test_loder)):
        ...
        correct_ops, mean_correct_adj, mean_false_positive_adj, correct_adj = utils.get_accuracy((ops_recon, adj_recon), (ops, adj))
    ...
    print(f'/n')
    print(f'Average loss of test set {epoch}: {avg_loss} | Op Acc:{correct_ops_ave} | Mean Correct Adj:{mean_correct_adj_ave} | Mean FP Adj: {mean_false_positive_adj_ave} | Adj Acc:{correct_adj_ave} | Cond loss:{pred_loss}')
...
def autoencoder_train(m1, trainloader, traindataset, testloader, 
    ...
    for epoch in range(1, 1+epoch_num):
        ...
        if lowest_loss is None or train_loss < lowest_loss:
            ...
            if save:
                torch.save(best_vgae_weight, os.path.join(save_path, 'm1_dist_{}.pt'.format(save_label)))
...
parser.add_argument('--batch_size', type=int, default=16, help='batch size of train loader')
