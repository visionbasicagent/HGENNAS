...
def construct_network_from_pickle(space_root, space, id, num_classes=10, se=True):
    f = open('{}/{}.pkl'.format(space_root, space), 'rb')
    data = pickle.load(f)
    dataset = pd.DataFrame(data)
    
    logging.info(dataset.iloc[id])
    adjs = dataset.iloc[id]['adj_matrix']
    opss = dataset.iloc[id]['ops_features']
    layer_channels = dataset.iloc[id]['layer_channels']
    layer_stride = dataset.iloc[id]['layer_stride']
    layer_repeats = dataset.iloc[id]['layer_repeats']
    final_concat_layer = dataset.iloc[id]['final_concat_layer']
    

    model = MasterModel(layer_channels, layer_stride, layer_repeats, final_concat_layer=final_concat_layer, adjs=adjs, opss=opss, num_classes=num_classes, se=se, stem_stride=2).model
    logging.info(model)
    return model
...
def main(opt, argv):
    ...
    # create model
    student_model = construct_network_from_pickle(opt.space_root, opt.space, opt.struct_id, opt.num_classes)
