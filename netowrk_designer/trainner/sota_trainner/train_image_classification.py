...
def construct_network_from_pickle(space_root, space, id, num_classes=10):
    ...
    model = MasterModel(layer_channels, layer_stride, layer_repeats, adjs=adjs, opss=opss, num_classes=num_classes).model.cuda()
    logging.info(model)
    return model
...
def main(opt, argv):
    ...
    # create model
    model = construct_network_from_pickle(opt.space_root, opt.space, opt.struct_id, opt.num_classes)
