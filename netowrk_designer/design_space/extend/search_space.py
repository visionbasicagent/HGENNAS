...
def process_op_features(ops, num_intermediate_nodes, num_canidate_ops, sample_skip):
    
    INPUT_OP = num_canidate_ops
    RESNET_OUTPUT_OP = num_canidate_ops+1
    GLOBAL_OP = num_canidate_ops+2
    
    features = np.zeros((num_intermediate_nodes+3, num_canidate_ops+3))
    kernel_features = []
    
    for i, op in enumerate(ops):    
        features[i, :][op]=1
        if op in [INPUT_OP, RESNET_OUTPUT_OP, GLOBAL_OP, 0]:
            kernel_features.append([0, 0, 0])
        else:
            random_kernel = random.randint(0, 2)
            k_f = [0, 0, 0]
            k_f[random_kernel] = 1
            kernel_features.append(k_f)
    ...
    if sample_skip:
        #remove skip
        features = features[:, 1:]
    return np.concatenate((features, kernel_features), axis=1)
...
class SearchSpace:
    ...
    def sample(self):
        ...
        INPUT_OP = self.num_canidate_ops
        RESNET_OUTPUT_OP = self.num_canidate_ops+1
        GLOBAL_OP = self.num_canidate_ops+2
        
        ops.insert(0, INPUT_OP)
        ops.insert(0, GLOBAL_OP)
        ops.append(RESNET_OUTPUT_OP)
                
        features = process_op_features(ops, self.num_intermediate_nodes, self.num_canidate_ops, self.sample_skip)
...
def sample_graphs(sample_size, notebook=False, seed=None, num_intermediate_node=6, num_candidate_ops=11, sample_skip=True, search_space=None):
    random.seed(seed)
    if search_space:
        ss = search_space
    else:
...
def calc_graph_hash(adj_matrix, ops_features):
    ...
    kernel_features = ops_features[:, -3:]
    ops_features = ops_features[:, :-3]
    ops_features = list(ops_features)   
    kernel_features = list(kernel_features)
    ops = ['_'.join([str(np.argmax(x)),str(np.argmax(y))]) if np.sum(x)!=0 else str(-1) for x, y in zip(ops_features, kernel_features) ]
    # print(adj_matrix)
    # print(ops)
...
def mutate_arch(adj_matrix, ops_features):
    ...
    if mutation_type == 'hidden_state_mutation':
        ...
        #do not mutate global and input node
        low = 2
        ...
        op_mutation_type = np.random.choice(['op_mutation', 'kernel_mutation'])
        if op_mutation_type == 'op_mutation':
            ...
            print('replace op {} from kernel {} to kernel {}'.format(random_op_idx, KERNELSET[original_kernel], KERNELSET[new_kernel]))
            ops_features_copy = np.array(ops_features_copy)
